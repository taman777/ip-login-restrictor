...
*   **デフォルトIP + 臨時IPで制御**: フロントエンドアクセス時に、グローバルな許可IPに加えて、そのページ専用の臨時IPも自動的にチェックされます。
*   **投稿一覧で確認可能**: 投稿一覧画面に「臨時IP」カラムが追加され、設定されているIPの数と内容を一目で確認できます。
...
2. サイドバーに表示される「IP Login Restrictor - 臨時IP設定」メタボックスを確認します
3. そのページ専用の臨時IPアドレスを1行に1つずつ入力します(CIDR表記も可能)
4. 「現在のIPを追加」ボタンで、今アクセスしているIPを簡単に追加できます
5. 投稿を更新すると、そのページへのアクセス時にデフォルトIP + 臨時IPの両方でチェックされます
